...
console.log(`Using loro-crdt@${LORO_VERSION()}`);
...
async function getCriticalBucketFactory(): Promise<IDBFactory | null> {
  ...
  if (!manager?.open) {
    if (!bucketFallbackLogged) {
      // eslint-disable-next-line no-console
      console.info("Storage Buckets API unavailable; using default IndexedDB.");
      bucketFallbackLogged = true;
    }
    return null;
  }
...
export async function withDocDb<T>(
...
export function fetchWorkspaceById(
  id: string,
): Promise<WorkspaceRecord | undefined> {
...
export async function deleteWorkspaceAndList(
  id: string,
