...
type WorkspaceTodoListProps = {
    ...
    onManualPointerDown: (cid: string, event: ReactPointerEvent<HTMLLIElement>) => void;
    onManualPointerMove: (cid: string, event: ReactPointerEvent<HTMLLIElement>) => void;
    onManualPointerUp: (cid: string, event: ReactPointerEvent<HTMLLIElement>) => void;
    onManualPointerCancel: (cid: string, event: ReactPointerEvent<HTMLLIElement>) => void;
    detached: boolean;
};
...
export function WorkspaceTodoList({
    ...
    const { stableTodos, indexByCid } = useMemo(() => {
        ...
        for (let i = 0; i < todos.length; i++) {
            indexed[todos[i].$cid] = i;
        }
        const sorted = [...todos].sort((a, b) => a.$cid.localeCompare(b.$cid));
        ...
        >
            {stableTodos.map((todo) => {
                ...
                const activeDragHeight =
                    activeDragCid != null
                        ? manualDrag?.height ?? itemHeights[activeDragCid] ?? defaultHeight
                        : defaultHeight;
                if (isManualActive && manualDrag) {
                    const rawY = manualDrag.clientY - manualDrag.listTop - manualDrag.offsetY;
                    const minY = -manualDrag.height * 0.6;
                    const maxY = Math.max(positions.height - manualDrag.height * 0.4, minY);
